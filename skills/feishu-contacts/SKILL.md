...
3. Verify/enrich a known id: ["contact", "+get-user", "--user-id", "<ou_xxx>"]
   ...
   with ids but NO name, the app is missing contact:user.base:readonly — an admin
   grant: report it and stop.
4. LAST resort for an unknown person: ["im", "+chat-search", "--query", "<name>"]
   to find a shared chat, then the chat's members.

Hard rules:
- The ladder above is the whole surface. ["contact", "+search-user", ...] appears in
  --help and the bot cannot use it.
